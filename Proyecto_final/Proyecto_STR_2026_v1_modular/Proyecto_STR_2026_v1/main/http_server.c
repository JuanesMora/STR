...
// Shared application context. It is provided by main.c before the server starts.
static app_context_t *g_app_ctx = NULL;

void http_server_set_app_context(app_context_t *ctx)
{
    g_app_ctx = ctx;
}
...
static esp_err_t http_server_read_json_body(httpd_req_t *req, cJSON **out_root)
{
    if (!req || !out_root) return ESP_FAIL;
    *out_root = NULL;
    ...
    while (received < content_length) {
        int ret = httpd_req_recv(req, buffer + received, content_length - received);
        if (ret <= 0) {
...
static void http_server_send_empty_ok(httpd_req_t *req)
{
    httpd_resp_set_type(req, "application/json");
    httpd_resp_set_hdr(req, "Connection", "close");
    httpd_resp_sendstr(req, "{\"ok\":true}");
...
static int json_get_int_default(cJSON *root, const char *key, int default_value)
{
...
static float json_get_float_default(cJSON *root, const char *key, float default_value)
{
    cJSON *item = cJSON_GetObjectItem(root, key);
    if (cJSON_IsNumber(item)) return (float)item->valuedouble;
    if (cJSON_IsString(item)) return (float)atof(item->valuestring);
    return default_value;
}

static bool json_get_bool_default(cJSON *root, const char *key, bool default_value)
{
    cJSON *item = cJSON_GetObjectItem(root, key);
    if (cJSON_IsBool(item)) return cJSON_IsTrue(item);
    if (cJSON_IsNumber(item)) return item->valueint != 0;
    if (cJSON_IsString(item)) return strcmp(item->valuestring, "true") == 0 || strcmp(item->valuestring, "1") == 0;
    return default_value;
}

static esp_err_t http_server_api_status_handler(httpd_req_t *req)
{
    if (!g_app_ctx) {
        httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "App context not available");
        return ESP_FAIL;
    }

    app_state_t state;
    if (!app_state_get_snapshot(g_app_ctx, &state)) {
        httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "Could not read app state");
        return ESP_FAIL;
    }

    cJSON *root = cJSON_CreateObject();
    cJSON_AddNumberToObject(root, "temperature", state.temperature_c);
    cJSON_AddNumberToObject(root, "adc_raw", state.adc_raw);
    cJSON_AddNumberToObject(root, "adc_voltage", state.adc_voltage_v);
    cJSON_AddNumberToObject(root, "temp_desired", state.temp_desired_c);
    cJSON_AddNumberToObject(root, "temp_max", state.temp_max_c);
    cJSON_AddStringToObject(root, "fan_mode", state.fan_mode == APP_FAN_MODE_AUTO ? "auto" : "manual");
    cJSON_AddNumberToObject(root, "fan_percent", state.fan_percent);
    cJSON_AddStringToObject(root, "curtain_mode", state.curtain_mode == APP_CURTAIN_MODE_AUTO ? "auto" : "manual");
    cJSON_AddNumberToObject(root, "curtain_percent", state.curtain_percent);
    cJSON_AddBoolToObject(root, "alarm", state.alarm_active);

    cJSON *rgb = cJSON_AddObjectToObject(root, "rgb");
    cJSON_AddNumberToObject(rgb, "r", state.rgb_r);
    cJSON_AddNumberToObject(rgb, "g", state.rgb_g);
    cJSON_AddNumberToObject(rgb, "b", state.rgb_b);
    cJSON_AddNumberToObject(rgb, "brightness", state.rgb_brightness);

    char *response = cJSON_PrintUnformatted(root);
    cJSON_Delete(root);

    httpd_resp_set_type(req, "application/json");
    httpd_resp_sendstr(req, response ? response : "{}");
    free(response);
    return ESP_OK;
}

static esp_err_t http_server_api_fan_handler(httpd_req_t *req)
{
    if (!g_app_ctx) return ESP_FAIL;
    cJSON *root = NULL;
    if (http_server_read_json_body(req, &root) != ESP_OK) return ESP_FAIL;

    app_state_t current;
    app_state_get_snapshot(g_app_ctx, &current);

    app_fan_mode_t mode = current.fan_mode;
    cJSON *mode_json = cJSON_GetObjectItem(root, "mode");
    if (cJSON_IsString(mode_json)) {
        mode = strcmp(mode_json->valuestring, "manual") == 0 ? APP_FAN_MODE_MANUAL : APP_FAN_MODE_AUTO;
    }

    float desired = json_get_float_default(root, "desired", current.temp_desired_c);
    float max = json_get_float_default(root, "max", current.temp_max_c);
    int percent = json_get_int_default(root, "percent", current.fan_percent);

    app_state_set_fan_config(g_app_ctx, mode, desired, max, app_clamp_percent(percent));
    cJSON_Delete(root);
    http_server_send_empty_ok(req);
    return ESP_OK;
}

static esp_err_t http_server_api_rgb_handler(httpd_req_t *req)
{
    if (!g_app_ctx) return ESP_FAIL;
    cJSON *root = NULL;
    if (http_server_read_json_body(req, &root) != ESP_OK) return ESP_FAIL;
    ...
    uint8_t r = app_clamp_percent(json_get_int_default(root, "r", current.rgb_r));
    uint8_t g = app_clamp_percent(json_get_int_default(root, "g", current.rgb_g));
    uint8_t b = app_clamp_percent(json_get_int_default(root, "b", current.rgb_b));
    uint8_t brightness = app_clamp_percent(json_get_int_default(root, "brightness", current.rgb_brightness));

    app_state_set_rgb(g_app_ctx, r, g, b, brightness);
    cJSON_Delete(root);
    http_server_send_empty_ok(req);
    return ESP_OK;
}

static esp_err_t http_server_api_curtain_handler(httpd_req_t *req)
{
    if (!g_app_ctx) return ESP_FAIL;
    cJSON *root = NULL;
    if (http_server_read_json_body(req, &root) != ESP_OK) return ESP_FAIL;
    ...
    app_curtain_mode_t mode = current.curtain_mode;
    cJSON *mode_json = cJSON_GetObjectItem(root, "mode");
    if (cJSON_IsString(mode_json)) {
        mode = strcmp(mode_json->valuestring, "auto") == 0 ? APP_CURTAIN_MODE_AUTO : APP_CURTAIN_MODE_MANUAL;
        app_state_set_curtain_mode(g_app_ctx, mode);
    }

    cJSON *percent_json = cJSON_GetObjectItem(root, "percent");
    if (cJSON_IsNumber(percent_json) || cJSON_IsString(percent_json)) {
        uint8_t percent = app_clamp_percent(json_get_int_default(root, "percent", current.curtain_percent));
        app_context_send_servo_command(g_app_ctx, percent, false);
    }
...
static esp_err_t http_server_api_schedule_get_handler(httpd_req_t *req)
{
    if (!g_app_ctx) return ESP_FAIL;

    app_state_t state;
    if (!app_state_get_snapshot(g_app_ctx, &state)) return ESP_FAIL;
    ...
    for (uint8_t i = 0; i < APP_CURTAIN_SCHEDULE_RECORDS; i++) {
        ...
        cJSON_AddBoolToObject(item, "enabled", state.schedule[i].enabled);
        cJSON_AddNumberToObject(item, "hour", state.schedule[i].hour);
        cJSON_AddNumberToObject(item, "minute", state.schedule[i].minute);
        cJSON_AddNumberToObject(item, "percent", state.schedule[i].percent);
    ...
    httpd_resp_set_type(req, "application/json");
    httpd_resp_sendstr(req, response ? response : "{\"items\":[]}");
    free(response);
    return ESP_OK;
}

static esp_err_t http_server_api_schedule_post_handler(httpd_req_t *req)
{
    if (!g_app_ctx) return ESP_FAIL;
    cJSON *root = NULL;
    if (http_server_read_json_body(req, &root) != ESP_OK) return ESP_FAIL;
    ...
    if (cJSON_IsArray(items)) {
        ...
        cJSON_ArrayForEach(item, items) {
            int index = json_get_int_default(item, "index", -1);
            if (index >= 0 && index < APP_CURTAIN_SCHEDULE_RECORDS) {
                int hour = json_get_int_default(item, "hour", 99);
                int minute = json_get_int_default(item, "minute", 99);
                curtain_schedule_record_t record = {
                    .enabled = json_get_bool_default(item, "enabled", false),
                    .hour = (uint8_t)hour,
                    .minute = (uint8_t)minute,
                    .percent = app_clamp_percent(json_get_int_default(item, "percent", 0)),
                };
                if (hour > 23 || minute > 59 || hour < 0 || minute < 0) record.enabled = false;
                app_state_set_schedule_record(g_app_ctx, (uint8_t)index, &record);
                app_storage_save_schedule_record((uint8_t)index, &record);
            }
        }
    } else {
        int index = json_get_int_default(root, "index", -1);
        if (index < 0 || index >= APP_CURTAIN_SCHEDULE_RECORDS) {
            cJSON_Delete(root);
        ...
        int hour = json_get_int_default(root, "hour", 99);
        int minute = json_get_int_default(root, "minute", 99);
        curtain_schedule_record_t record = {
            .enabled = json_get_bool_default(root, "enabled", true),
            .hour = (uint8_t)hour,
            .minute = (uint8_t)minute,
            .percent = app_clamp_percent(json_get_int_default(root, "percent", 0)),
        };
        if (hour > 23 || minute > 59 || hour < 0 || minute < 0) record.enabled = false;
        app_state_set_schedule_record(g_app_ctx, (uint8_t)index, &record);
        app_storage_save_schedule_record((uint8_t)index, &record);
...
static esp_err_t http_server_ap_config_json_handler(httpd_req_t *req)
{
    cJSON *root = NULL;
    if (http_server_read_json_body(req, &root) != ESP_OK) return ESP_FAIL;

    cJSON *ssid_json = cJSON_GetObjectItem(root, "ap_ssid");
    cJSON *pass_json = cJSON_GetObjectItem(root, "ap_password");
    if (!cJSON_IsString(ssid_json) || !cJSON_IsString(pass_json)) {
        cJSON_Delete(root);
        httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "Missing ap_ssid or ap_password");
        return ESP_FAIL;
    }

    esp_err_t err = wifi_app_set_softap_credentials(ssid_json->valuestring, pass_json->valuestring);
    cJSON_Delete(root);

    if (err != ESP_OK) {
        httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "Could not update AP credentials");
        return ESP_FAIL;
    }
...
static httpd_handle_t http_server_configure(void)
{
	...
	httpd_register_uri_handler(http_server_handle, &dht_sensor_json);
        ...
        httpd_uri_t api_status = {
                .uri = "/api/status",
                .method = HTTP_GET,
                .handler = http_server_api_status_handler,
                .user_ctx = NULL
        };
        ...
        httpd_uri_t api_fan = {
                .uri = "/api/fan",
                .method = HTTP_POST,
                .handler = http_server_api_fan_handler,
                .user_ctx = NULL
        };
        ...
        httpd_uri_t api_rgb = {
                .uri = "/api/rgb",
                .method = HTTP_POST,
                .handler = http_server_api_rgb_handler,
                .user_ctx = NULL
        };
        httpd_register_uri_handler(http_server_handle, &api_rgb);

        httpd_uri_t api_curtain = {
                .uri = "/api/curtain",
                .method = HTTP_POST,
                .handler = http_server_api_curtain_handler,
                .user_ctx = NULL
        };
        httpd_register_uri_handler(http_server_handle, &api_curtain);

        httpd_uri_t api_schedule_get = {
                .uri = "/api/schedule",
                .method = HTTP_GET,
                .handler = http_server_api_schedule_get_handler,
                .user_ctx = NULL
        };
        httpd_register_uri_handler(http_server_handle, &api_schedule_get);

        httpd_uri_t api_schedule_post = {
                .uri = "/api/schedule",
                .method = HTTP_POST,
                .handler = http_server_api_schedule_post_handler,
                .user_ctx = NULL
        };
        httpd_register_uri_handler(http_server_handle, &api_schedule_post);

        httpd_uri_t ap_config_json = {
                .uri = "/apConfig.json",
                .method = HTTP_POST,
                .handler = http_server_ap_config_json_handler,
                .user_ctx = NULL
        };
        httpd_register_uri_handler(http_server_handle, &ap_config_json);
