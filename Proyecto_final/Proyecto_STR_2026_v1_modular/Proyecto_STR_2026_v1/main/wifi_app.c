...
static esp_err_t save_softap_credentials(const char *ssid, const char *password)
{
    nvs_handle_t nvs_handle;
    ...
    if (err == ESP_OK) err = nvs_set_str(nvs_handle, "ap_password", password);
...
static bool load_softap_credentials(char *ssid, size_t ssid_len, char *password, size_t password_len)
{
    nvs_handle_t nvs_handle;
    ...
    err = nvs_get_str(nvs_handle, "ap_ssid", ssid, &required);
    if (err != ESP_OK) {
        nvs_close(nvs_handle);
        return false;
    }
    ...
    err = nvs_get_str(nvs_handle, "ap_password", password, &required);
    nvs_close(nvs_handle);
    return err == ESP_OK;
}

esp_err_t wifi_app_set_softap_credentials(const char *ssid, const char *password)
{
    ...
    if (ssid_len == 0 || ssid_len > MAX_SSID_LENGTH) return ESP_ERR_INVALID_ARG;
    if (pass_len > 0 && pass_len < 8) return ESP_ERR_INVALID_ARG;
    if (pass_len > MAX_PASSWORD_LENGTH) return ESP_ERR_INVALID_ARG;
    ...
    esp_err_t err = save_softap_credentials(ssid, password);
    if (err != ESP_OK) return err;

    err = esp_wifi_set_config(WIFI_IF_AP, &ap_config);
    if (err == ESP_OK) {
        ESP_LOGI(TAG, "SoftAP updated. SSID: %s", ssid);
    }
...
static void wifi_app_soft_ap_config(void)
{
	...
	load_softap_credentials(ap_ssid, sizeof(ap_ssid), ap_password, sizeof(ap_password));
...
static void wifi_app_task(void *pvParameters)
{
	...
	// initialize_registers(); // Agenda antigua del ejemplo base. La agenda nueva esta en app_state/app_tasks.
	...
	/*
				...
				case WIFI_APP_MSG_START_HTTP_SERVER:
					...
					http_server_start();
					// RGB ambiental reservado para el usuario.
...
void wifi_app_start(void)
{
	...
	// RGB ambiental reservado para rgb_ctrl.c. No se usa como LED de estado WiFi.
	...
	// La tarea antigua de agenda del ejemplo queda desactivada.
	// La agenda de cortinas se ejecuta en task_schedule_control() dentro de app_tasks.c.
